...
from src.processors.image.alignment.piecewise_affine_delaunay import (
    apply_piecewise_affine,
)
from src.processors.image.alignment.utils import show_displacement_overlay
...
class SiftMatcher:
    ...
    def get_matches(
        ...
        logger.debug(
            f"SIFT: {field_block_name}: alignment_image", np.histogram(alignment_image)
        )
        logger.debug(f"SIFT: {field_block_name}: gray_image", np.histogram(gray_image))
