...
def _get_base_config() -> dict:
    """Get a minimal base config for testing.

    Note: Defaults are merged in by open_config_with_defaults(),
    so we only need to provide the fields we're testing.
    """
    return {}


def _validate_config_with_defaults(config_dict, config_path):
...
def test_show_image_level_with_max_parallel_workers_validation(tmp_path) -> None:
    ...
    # Use camelCase keys as expected by the schema
    invalid_config = deepcopy(_get_base_config())
    invalid_config["outputs"] = {"showImageLevel": 1}  # Interactive mode
    invalid_config["processing"] = {
        "maxParallelWorkers": 4  # Should be 1 when showImageLevel > 0
    }
    # ruff: noqa: PT011
...
def test_custom_error_message_for_parallel_workers_validation(tmp_path, capsys) -> None:
    ...
    # Use camelCase keys as expected by the schema
    invalid_config = deepcopy(_get_base_config())
    invalid_config["outputs"] = {"showImageLevel": 2}  # Interactive mode
    invalid_config["processing"] = {
        "maxParallelWorkers": 8  # Invalid: should be 1
    }
    ...
    assert "showImageLevel > 0" in output or "interactive mode" in output
    assert "maxParallelWorkers must be 1" in output
    assert (
